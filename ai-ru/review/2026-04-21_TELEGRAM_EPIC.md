...
**Статус:** ✅ Completed (2026-04-23) · **Заведён:** 2026-04-21 · **Связан с:** [BF1](../backlog/BF1_TELEGRAM_BOT_UI.md), [ROADMAP Phase 2](../ROADMAP.md), [TECH_DEBT A1–A4](../TECH_DEBT.md)
...
| **2.3 ✅** | **Сообщить о событии + Поддержка + Маркетплейс.** Scene'ы `ReportEventScene` (многошаговая форма через `SceneStateRegistry`), `SupportScene` (текст → feedback, `FeedbackChannels.TELEGRAM`), `MarketplaceScene` (URL-кнопки Ozon/WB). `TelegramScene.handleText` default-метод + router обрабатывает pending-text-state. | Да | [spec](2026-04-21_TG_2.3_USER_ACTIONS.md) · [plan](2026-04-21_TG_2.3_PLAN.md) |
| **2.4 ✅** | **Deep-link онбординг.** Мобайл: кнопка «Подключить бота» → `POST /api/telegram.get_start_url` → `https://t.me/<bot>?start=<HMAC_token>` (TTL 15 мин). Бэк: `TelegramStartTokenService` (HMAC-SHA256, ≤64 символа), `TelegramAuthController`, роутер отсекает `/start ` префикс, auth-service верифицирует и привязывает без shareContact. Мобайл: `WidgetsBindingObserver` автоматически рефрешит настройки при возврате. | Да (и в мобайле) | [spec](2026-04-21_TG_2.4_DEEP_LINK.md) · [plan](2026-04-21_TG_2.4_PLAN.md) |
| **2.5 ✅** | **Уведомления-карточки + финал.** `NotificationMarkReadScene.renderNotification` строит HTML-карточку: эмодзи-по-reason + имя метки + reason + текст пользователя + время. Кнопки «✓ Отметить» + «🚘 К метке». `HomeScene.KEY` константа (убран литерал из роутера). Javadoc в `TelegramScene` с примером для Phase 5. | Да | [spec](2026-04-21_TG_2.5_NOTIFICATION_CARD.md) · [plan](2026-04-21_TG_2.5_PLAN.md) |
...
- 2026-04-23 — **2.3 закрыт**. `ReportEventScene` (4-step form), `SupportScene` (one-step feedback), `MarketplaceScene` (WB+Ozon url-кнопки). `SceneStateRegistry` (in-memory, TTL 5 мин). `TelegramScene.handleText` default + router pending-state check. `HomeScene` +3 кнопки, `QrDetailsScene` +кнопка «📝 Событие». `FeedbackChannels.TELEGRAM` канал уже был в enum.
- 2026-04-23 — **2.4 закрыт**. `TelegramStartTokenService` (HMAC-SHA256, truncated до 80 бит, TTL 15 мин, fits 64-char Telegram budget). `TelegramAuthController.POST /api/telegram.get_start_url`. Router отсекает `/start` префикс, auth-service обрабатывает signed token и привязывает без shareContact. Мобайл: `NotificationsRepository.getTelegramStartUrl` + controller `launchTelegramBot` + `WidgetsBindingObserver` auto-refresh при возврате.
- 2026-04-23 — **2.5 закрыт, эпик завершён ✅**. `NotificationMarkReadScene.renderNotification` переписан: HTML-карточка с эмодзи-по-reason (i18n-маппинг `tg.reason.emoji.<id>`, 8 reasons + default `🚗`), имя метки, описание причины, пользовательский текст в кавычках-ёлочках, дата через `DateTimeFormatter` в локали `ru`. Две кнопки: «✓ Отметить прочитанным» и «🚘 К метке» (→ `qr_details:open:<qrId>`). Fallback на plain-text если `Notification` не загрузилась. Финализация архитектуры: `HomeScene.KEY` константа вместо литерала в `TelegramRouter`, Javadoc в `TelegramScene` с примером добавления новой сцены под Phase 5.
