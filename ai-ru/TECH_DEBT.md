...
| P11 | ~~**`car_id.jar` собран 4 ноября 2025**, миграции 1.9–1.17 и наши доработки туда не попали. 5+ месяцев без деплоев.~~ **Закрыто 2026-06-27** — выкачен новый бэкенд (Тир-2 `call_enabled` + фикс `findByQrId`), Liquibase накатил недостающую 1.17.1 (`DROP whatsapp_enabled`). На момент деплоя prod-БД фактически была уже на 1.16.2 (а не на 1.8, как считалось). | 🟡 → ✅ | [F8](features/F8_TELEGRAM_BOT.md) |
...
| P17 | **Telegram кладёт старт прода + сейчас отключён.** `api.telegram.org` недоступен с prod-сервера `79.174.94.70` (блокировка; общая сеть жива). Регистрация бота синхронная в `@PostConstruct` (`TelegramConfig.initTelegramBot` → `TelegramBotService.init` → `registerBot`) → при `telegram.enable=true` старт падает (`Application run failed: Network is unreachable`), systemd уходит в crash-loop. **2026-06-27 выставлен `telegram.enable: false`** в `/var/www/car_id/application-prod.yml` (бэкап `*.bak-predeploy-20260627`), чтобы поднять сайт после деплоя Тир-2 — Telegram-уведомления отключены. **Двойная задача:** (1) восстановить доступ к Telegram (прокси/VPN на исходящие) и вернуть `enable=true`; (2) сделать регистрацию бота неблокирующей (async + `try/catch`), чтобы недоступность Telegram не валила старт. Также `run.sh` через `send_message.sh` бьётся в Telegram на старте → +~2 мин к каждому рестарту (таймаут). | 🟠 | [F8](features/F8_TELEGRAM_BOT.md), [`../deploy/run.sh`](../deploy/run.sh) |
