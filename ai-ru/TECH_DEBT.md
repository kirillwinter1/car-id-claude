...
| P17 | ~~**Telegram кладёт старт прода + отключён.** `api.telegram.org` заблокирован с prod-сервера `79.174.94.70` (РФ).~~ **Решено 2026-06-27** — `sing-box.service` (systemd, VLESS-Reality, узел NL) поднимает локальный SOCKS5 `127.0.0.1:10808`; `TelegramBotService` через `DefaultBotOptions` гонит **только** Telegram-трафик через прокси (`telegram.proxy.enabled=true`), остальное напрямую. `telegram.enable=true`, бот живой (`getMe` ok). Конфиг прокси — `/etc/sing-box/config.json` (chmod 600, вне git). **Остаток:** (1) `run.sh`→`send_message.sh` бьётся в Telegram **напрямую** (не через прокси) → +~2 мин к рестарту; (2) регистрация бота всё ещё блокирующая в `@PostConstruct` (async+catch не сделан); (3) нет failover на другой узел; (4) sid в конфиге может протухнуть (подписка ротирует). | 🟠 → 🟡 | [F8](features/F8_TELEGRAM_BOT.md), [`../deploy/systemd/sing-box.service`](../deploy/systemd/sing-box.service) |
